// maa-ctrl-playtools exports.cpp
// Wraps PlayToolsController behind the maa_ctrl_* C-ABI.
...
{
    // TODO: Integrate PlayToolsImpl (PlayToolsController) here.
    (void)adb_path; (void)address; (void)config_json;
    auto* state = new PlayToolsState();
    state->last_error = "maa-ctrl-playtools: not yet fully implemented";
...
{
    if (!handle) return;
    delete reinterpret_cast<PlayToolsState*>(handle);
}

EXPORT bool maa_ctrl_inited(const MaaCtrlHandle* handle)
{
    if (!handle) return false;
    return reinterpret_cast<const PlayToolsState*>(handle)->initialized;
}

EXPORT const char* maa_ctrl_get_uuid(const MaaCtrlHandle* handle)
{
    if (!handle) return nullptr;
    return reinterpret_cast<const PlayToolsState*>(handle)->uuid.c_str();
}
...
EXPORT int64_t maa_ctrl_support_features(const MaaCtrlHandle* handle) { (void)handle; return MAA_CTRL_FEAT_NONE; }
...
{
    if (!handle) return nullptr;
    return reinterpret_cast<const PlayToolsState*>(handle)->last_error.c_str();
}

EXPORT const char* maa_ctrl_version(void)
{
    return "maa-ctrl-playtools 1.0.0";
}
