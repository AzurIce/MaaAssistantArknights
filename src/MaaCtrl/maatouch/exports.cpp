...
{
    // TODO: Integrate MaatouchController (MinitouchController with m_use_maa_touch=true) here.
    (void)adb_path; (void)address; (void)config_json;
    auto* state = new MaatouchState();
    state->last_error = "maa-ctrl-maatouch: not yet fully implemented";
...
{
    if (!handle) return;
    delete reinterpret_cast<MaatouchState*>(handle);
}

EXPORT bool maa_ctrl_inited(const MaaCtrlHandle* handle)
{
    if (!handle) return false;
    return reinterpret_cast<const MaatouchState*>(handle)->initialized;
}

EXPORT const char* maa_ctrl_get_uuid(const MaaCtrlHandle* handle)
{
    if (!handle) return nullptr;
    return reinterpret_cast<const MaatouchState*>(handle)->uuid.c_str();
}
...
{
    if (!handle) return nullptr;
    return reinterpret_cast<const MaatouchState*>(handle)->last_error.c_str();
}

EXPORT const char* maa_ctrl_version(void)
{
    return "maa-ctrl-maatouch 1.0.0";
}
