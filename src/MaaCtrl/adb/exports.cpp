...
// Stub exports — the actual implementation lives in adb-base (AdbBase).
// Full integration requires linking AdbBase and its dependencies.
// For now, provide the C-ABI skeleton.
...
{
    // TODO: Integrate AdbBase (AdbController) here.
    // For now, create a placeholder state.
    (void)adb_path; (void)address; (void)config_json;
    auto* state = new AdbState();
    state->last_error = "maa-ctrl-adb: not yet fully implemented";
...
{
    if (!handle) return;
    delete reinterpret_cast<AdbState*>(handle);
}

EXPORT bool maa_ctrl_inited(const MaaCtrlHandle* handle)
{
    if (!handle) return false;
    return reinterpret_cast<const AdbState*>(handle)->initialized;
}

EXPORT const char* maa_ctrl_get_uuid(const MaaCtrlHandle* handle)
{
    if (!handle) return nullptr;
    return reinterpret_cast<const AdbState*>(handle)->uuid.c_str();
}
...
{
    (void)handle;
    // ADB controller has imprecise swipe
    return MAA_CTRL_FEAT_IMPRECISE_SWIPE;
}
...
EXPORT bool maa_ctrl_get_image(const MaaCtrlHandle* handle, uint32_t* w, uint32_t* h, const uint8_t** data, uint32_t* len)
{
    (void)handle; (void)w; (void)h; (void)data; (void)len;
    return false;
}
...
{
    if (!handle) return nullptr;
    return reinterpret_cast<const AdbState*>(handle)->last_error.c_str();
}

EXPORT const char* maa_ctrl_version(void)
{
    return "maa-ctrl-adb 1.0.0";
}
